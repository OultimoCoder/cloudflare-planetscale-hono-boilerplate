...
export const spotifyRedirect: Handler<{ Bindings: Bindings }> = async (c) => {
...
export const spotifyCallback: Handler<{ Bindings: Bindings }> = async (c) => {
  const config = getConfig(c.env)
...
export const linkSpotify: Handler<{ Bindings: Bindings }> = async (c) => {
  const config = getConfig(c.env)
  const request = await validateCallbackBody(c)
  const oauthRequest = spotify.users({
    options: {
...
export const deleteSpotifyLink: Handler<{ Bindings: Bindings }> = async (c) => {
